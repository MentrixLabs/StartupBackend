...
def add_text_to_rectangles(
    ...
    """Добавляет текст в пустые прямоугольники на изображении."""
...
def create_infographics(image, texts):
    # уменьшить обект в scale_factor раз
    image = resize_object_centered(image, scale_factor=0.7)
    # маска прозрачной картинки
    mask = get_alpha_mask(image)
    # поиск прямоугольников (они не пересекаются)
    empty_rects = find_empty_rectangles(mask)
    ...
    print(check_no_overlap(empty_rects))
    # добавить текст
    result = add_text_to_rectangles(
        image,
        empty_rects,
        texts,
        font_path="./assets/Montserrat-Medium.ttf",
        font_size=150,
        text_color=(0, 0, 0), # Белый текст
        padding=15
    )
    return result
